...
export interface ISong extends BaseEntity {
  ...
  releaseDate: Date;
  coverArtUrl: string;
  audioFileKey: string; // S3/storage key for HLS manifest
...
export interface IListeningHistory extends BaseEntity {
  ...
  artistName: string;
  listenedAt: Date;
  duration: number; // how long user actually listened (seconds)
...
export interface IDailyMetrics {
  ...
  topSongs: Array<{ songId: string; title: string; plays: number }>;
