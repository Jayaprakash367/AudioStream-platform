...
async function buildApp(config) {
    ...
    const redis = new redis_client_1.RedisCacheManager({ host: config.redisHost, port: config.redisPort, password: config.redisPassword, keyPrefix: 'music:' });
    await redis.connect();
    ...
    const musicService = new services_1.MusicService(redis);
    await (0, routes_1.registerMusicRoutes)(app, musicService);
    // Consume song.played events to increment play counts
    await kafka.subscribe('song.played', async (payload) => {
